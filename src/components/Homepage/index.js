    ...
    useEffect(() => {
        ...
        class Particle {
            constructor(canvas, options) {
                ...
                // x,y 起始坐标
                this.x = (windowWidth / 2) + (Math.random() * 200 - Math.random() * 200)
                this.y = (windowHeight / 2) + (Math.random() * 200 - Math.random() * 200)
                this.w = windowWidth
                this.h = windowHeight
        ...
        function clear() {
            ...
            canvas.fillStyle = colorMode !== 'light' ? '#000' : '#fff';
            canvas.fillRect(0, 0, tela.width, tela.height);
        }

        function connection() {
            let old_element = null
            particles.map((element, i) => {
                if (i > 0) {
